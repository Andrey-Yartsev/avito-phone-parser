...
class AGI
{
    ...
    */
    function __construct($config=NULL, $optconfig=array())
    {
        ...
        if(!isset($this->config['phpagi']['debug'])) $this->config['phpagi']['debug'] = true;
  ...
  function getDigit($file, $length = 1) {
    $this->conlog("Try to get digit by sound '$file'");
    return $this->fastpass_get_data($buffer, $file, 10000, $length);
  }

  function getVar($k) {
    return $this->get_variable($k)['data'];
  }

  function playback($file) {
    if ($this->request['agi_extension'] == 'h') return;
    $this->exec('Playback', $file);
  }
